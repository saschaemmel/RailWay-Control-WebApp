            ...
            if(true == $setup){
                //if ($result->num_rows > 0) {
                    echo('<div class="divTableCell" style="width:'.$gridsize.'px; height:'.$gridsize.'px;">
                            <a href="/overview.php?setup=true&posx='.$posx.'&posy='.$posy.'">
                                <img src="images/' .$row["pic"]. '.png" width="'.$gridsize.'" height="'.$gridsize.'" alt="Image not Found!" style="transform:rotate('.$row["angle"].'deg);"/>
                            </a>
                        </div>'."\n");
                //}
                //else{
...
<?php
    ...
    if (isset($_POST['aktion']) and $_POST['aktion']=='speichern') {
        ...
        $sql = "UPDATE scheme SET pic='$signal', angle='$rotation', flipx='0', flipy='0' 
            WHERE posx='$tmpPosx' AND posy='$tmpPosy'";
